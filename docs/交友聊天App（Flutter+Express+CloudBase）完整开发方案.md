...
# 三、后端技术方案（Express + CloudBase 国内最新流行适配版）
...
3.1.1 Express生产级项目目录结构（适配CloudBase部署+前端需求）
遵循Express轻量灵活的特性，结合交友App四大核心业务场景，设计分层清晰、职责明确的生产级目录结构，支持高可维护性、可扩展性，适配CloudBase CloudRun部署、Docker容器化，无缝对接Prisma、Socket.IO等技术栈，具体结构如下（注释清晰，便于开发与后期迭代）：

dating_app_api/
├── src/                      # 核心代码目录（Express项目核心）
│   ├── app.js                # 入口文件，初始化Express应用、配置中间件、连接数据库与缓存
│   ├── routes/               # 路由目录（按业务模块划分，统一管理接口路由）
│   │   ├── index.js          # 路由入口，聚合所有业务路由，注册至Express
│   │   ├── user.routes.js    # 用户管理路由（注册、登录、好友关系等接口）
│   │   ├── im.routes.js      # IM即时通讯路由（消息存储、已读、撤回等接口）
│   │   ├── game.routes.js    # 你画我猜游戏路由（房间管理、笔触同步等接口）
│   │   ├── slow-chat.routes.js # 慢社交路由（慢消息发送、开封等接口）
│   │   ├── circle.routes.js  # 兴趣圈子路由（发帖、评论、点赞等接口）
│   │   └── common.routes.js  # 通用公共路由（文件上传、Token刷新等接口）
│   ├── controllers/          # 控制器目录（处理接口请求、返回响应，与路由对应）
│   │   ├── user.controller.js # 用户控制器，处理用户相关接口逻辑
│   │   ├── im.controller.js  # IM控制器，处理IM相关接口逻辑
│   │   ├── game.controller.js # 游戏控制器，处理游戏相关接口逻辑
│   │   ├── slow-chat.controller.js # 慢社交控制器，处理慢消息相关逻辑
│   │   ├── circle.controller.js # 圈子控制器，处理圈子相关接口逻辑
│   │   └── common.controller.js # 公共控制器，处理通用接口逻辑
│   ├── services/             # 服务层目录（封装核心业务逻辑，与控制器对应）
│   │   ├── user.service.js   # 用户服务，实现用户相关业务逻辑（与Prisma交互）
│   │   ├── im.service.js     # IM服务，实现IM相关业务逻辑
│   │   ├── game.service.js   # 游戏服务，实现游戏相关业务逻辑
│   │   ├── slow-chat.service.js # 慢社交服务，实现慢消息相关业务逻辑
│   │   ├── circle.service.js # 圈子服务，实现圈子相关业务逻辑
│   │   └── common.service.js # 公共服务，实现通用业务逻辑（文件上传、敏感词过滤等）
│   ├── middleware/           # 中间件目录（全局复用，拦截请求/响应）
│   │   ├── auth.middleware.js # 鉴权中间件（JWT校验，保护需要登录的接口）
│   │   ├── validation.middleware.js # 参数校验中间件（校验请求参数合法性）
│   │   ├── error.middleware.js # 异常处理中间件（统一捕获接口异常，返回标准错误）
│   │   ├── logging.middleware.js # 日志中间件（记录请求、响应日志，便于排查）
│   │   └── cors.middleware.js # 跨域中间件（配置跨域规则，适配前端请求）
│   ├── config/               # 全局配置目录（统一管理第三方服务参数、环境变量）
│   │   ├── index.js          # 配置入口，导出所有配置
│   │   ├── db.config.js      # 数据库配置（MySQL 8.0、Prisma配置）
│   │   ├── redis.config.js   # Redis配置（Upstash Redis连接参数）
│   │   ├── cos.config.js     # 腾讯云COS配置（文件上传参数）
│   │   ├── im.config.js      # 腾讯IM配置（SDK参数，对接前端IM SDK）
│   │   ├── push.config.js    # 推送配置（极光/TPNS参数）
│   │   └── server.config.js  # 服务配置（端口、环境、跨域等）
│   ├── utils/                # 公共工具类目录（全局复用，简化开发）
│   │   ├── encryption.util.js # 加密工具（bcrypt加密、数据加密）
│   │   ├── redis.util.js     # Redis工具（封装缓存增删改查操作）
│   │   ├── cos.util.js       # 腾讯云COS工具（文件上传/下载封装）
│   │   ├── push.util.js      # 推送工具（极光/TPNS推送封装）
│   │   ├── common.util.js    # 通用工具（日期、格式转换、敏感词过滤等）
│   │   └── logger.util.js    # 日志工具（集成winston，统一日志格式）
│   ├── prisma/               # Prisma数据库模块（适配MySQL 8.0）
│   │   ├── schema.prisma     # 数据库schema配置（完整数据模型）
│   │   └── prisma.js         # Prisma客户端初始化，导出供服务层调用
│   ├── socket/               # Socket.IO实时交互目录（处理实时通信逻辑）
│   │   ├── index.js          # Socket初始化，关联Express应用，注册事件
│   │   ├── game.socket.js    # 游戏Socket逻辑（笔触同步、房间通知等）
│   │   └── im.socket.js      # IM Socket逻辑（实时消息推送、在线状态同步等）
│   └── types/                # 类型定义目录（TypeScript，统一类型约束）
│       ├── global.types.js   # 全局通用类型
│       └── response.types.js # 统一响应类型（适配前端）
...
├── .env.test                 # 测试环境配置
├── package.json              # 依赖配置（Express插件、第三方库）
...
目录结构说明：该结构遵循Express分层开发规范，按“路由-控制器-服务层”分层，职责明确、低耦合，契合Express轻量灵活的特性；Socket.IO实时交互单独拆分目录，便于统一管理实时通信逻辑；Prisma模块单独封装，确保数据操作的一致性；全局配置、工具类、中间件等公共资源集中管理，提升复用性；适配Docker容器化与CloudBase CloudRun部署，可直接打包部署，无需额外修改目录结构；支持TypeScript类型约束，提升代码可靠性与可维护性，完美适配生产级开发需求，同时无缝对接前端Flutter应用的接口调用规范。
